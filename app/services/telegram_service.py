...
"""
    ...
    async def check_user_subscription(self, user_id: int) -> bool:
        ...
        try:
            ...
            logger.info(f"🔍 Проверяем подписку пользователя {user_id} на группу {channel_id}")
            ...
            logger.info(f"Проверка подписки пользователя {user_id} на группу {channel_id}: статус '{chat_member.status}', подписан: {is_subscribed}")
            return is_subscribed
            
        except TelegramError as e:
            logger.error(f"Ошибка проверки подписки пользователя {user_id} на группу {channel_id}: {e}")
    ...
    async def send_subscription_required_message(self, user_id: int) -> bool:
        ...
        message = """
🔒 <b>Требуется участие в группе</b>
